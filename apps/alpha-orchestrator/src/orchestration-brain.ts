...
export class OrchestrationBrain extends DurableObject {
  ...
  private async handleEventLog(request: Request): Promise<Response> {
    ...
    return new Response(JSON.stringify(recentEvents), { headers: { "Content-Type": "application/json" } });
  }
  ...
  private async handleTransition(request: Request): Promise<Response> {
    ...
    const result = await this.ctx.storage.transaction(async (txn) => {
      ...
      if (!validation.valid) {
        return { success: false, error: validation.error, details: { message: validation.message }, newVersion: state.version, eventLogged: false };
      }
      ...
      if (currentAgentState && currentAgentState.status !== event.transition.fromState) {
        return { success: false, error: "InvalidFromState", details: { expected: event.transition.fromState, actual: currentAgentState.status }, newVersion: state.version, eventLogged: false };
      }
      ...
      events.push({ eventId: event.eventId, correlationId: event.correlationId, timestamp: event.timestamp, sourceAgent: event.sourceAgent, transition: event.transition as any, version: state.version, metadata: event.payload });
  ...
  private async handleTransitionIdempotent(request: Request): Promise<Response> {
    ...
    if (events?.some(e => e.eventId === event.eventId)) {
      const state = await this.ctx.storage.get<SystemStateSnapshot>(STORAGE_STATE_KEY);
      return new Response(JSON.stringify({ success: true, newVersion: state?.version || 0, eventLogged: false, details: { idempotent: true, alreadyProcessed: event.eventId } }), {
  ...
  private async handleLock(request: Request): Promise<Response> {
    ...
    if (action === "status" && correlationId) {
      ...
      return new Response(JSON.stringify({ locked: !!lock && Date.now() <= lock.expiresAt, lock: lock && Date.now() <= lock.expiresAt ? lock : null }), { headers: { "Content-Type": "application/json" } });
    }

    if (action === "release" && correlationId) {
      const lock = state.activeLocks[correlationId];
      if (!lock) {
        return new Response(JSON.stringify({ released: false, reason: "NoLock" }), { headers: { "Content-Type": "application/json" } });
      }
      if (lock.heldBy !== agentIdentity) {
        return new Response(JSON.stringify({ released: false, reason: "LockOwnership", heldBy: lock.heldBy }), { status: 403, headers: { "Content-Type": "application/json" } });
      }
      await this.ctx.storage.transaction(async (txn) => {
        const s = await txn.get<SystemStateSnapshot>(STORAGE_STATE_KEY);
        if (s) { delete s.activeLocks[correlationId]; await txn.put(STORAGE_STATE_KEY, s); }
      });
      return new Response(JSON.stringify({ released: true }), { headers: { "Content-Type": "application/json" } });
    }
      ...
      const result = await this.ctx.storage.transaction(async (txn) => {
        ...
        if (currentState.activeLocks[correlationId!]) {
          ...
          if (Date.now() > existingLock.expiresAt) {
            ...
            return { acquired: false, reason: "LockHeld", heldBy: existingLock.heldBy, expiresAt: existingLock.expiresAt };
          }
        }
        currentState.activeLocks[correlationId!] = { heldBy: agentIdentity, acquiredAt: Date.now(), expiresAt: Date.now() + ttl, correlationId: correlationId! };
        await txn.put(STORAGE_STATE_KEY, currentState);
        return { acquired: true, expiresAt: Date.now() + ttl };
      });
      return new Response(JSON.stringify(result), { status: result.acquired ? 200 : 409, headers: { "Content-Type": "application/json" } });
    }

    return new Response(JSON.stringify({ activeLocks: state.activeLocks }), { headers: { "Content-Type": "application/json" } });
  }
  ...
  private async handleReconcile(): Promise<Response> {
    const result = await this.ctx.storage.transaction(async (txn) => {
      ...
      for (const [key, lock] of Object.entries(state.activeLocks)) {
        if (now > lock.expiresAt) { delete state.activeLocks[key]; cleanedLocks++; }
      }
      const lockCount = Object.keys(state.activeLocks).length;
      if (lockCount > 50) state.status = "degraded";
      else if (lockCount === 0 && Object.keys(state.agentStates).length > 0) state.status = "healthy";
      state.lastReconciliation = now;
      await txn.put(STORAGE_STATE_KEY, state);
      return { reconciled: true, status: state.status, activeLocks: lockCount, cleanedLocks, version: state.version };
    });
    return new Response(JSON.stringify(result), { headers: { "Content-Type": "application/json", "X-Version": String(result.version || 0) } });
  }
  ...
  private async handleSnapshot(): Promise<Response> {
    ...
    const events = await this.ctx.storage.get<EventLogEntry[]>(STORAGE_EVENT_LOG_KEY);
    return new Response(JSON.stringify({ snapshot: state, eventCount: events?.length || 0, latestEventId: events?.[events.length - 1]?.eventId }), {
      headers: { "Content-Type": "application/json", "X-Version": String(state.version) }
    });
  }
  ...
  private validateTransition(event: AgentTransitionEvent): { valid: boolean; error?: string; message?: string } {
    ...
    if (!VALID_TRANSITIONS[fromState]) return { valid: false, error: "InvalidState", message: `Unknown fromState: ${fromState}` };
    if (!VALID_TRANSITIONS[toState]) return { valid: false, error: "InvalidState", message: `Unknown toState: ${toState}` };
    if (!VALID_TRANSITIONS[fromState].includes(toState)) {
      return { valid: false, error: "InvalidTransition", message: `Illegal transition: ${fromState} -> ${toState}. Valid: ${VALID_TRANSITIONS[fromState].join(", ") || "none"}` };
    }
    return { valid: true };
  }
  ...
  private cleanStaleLocksSync(state: SystemStateSnapshot): SystemStateSnapshot {
    const now = Date.now();
    const cleaned = { ...state, activeLocks: {} };
    for (const [key, lock] of Object.entries(state.activeLocks)) {
  ...
  private errorResponse(error: string, status: number, message?: string): Response {
    return new Response(JSON.stringify({ error, ...(message && { message }) }), { status, headers: { "Content-Type": "application/json" } });
  }
}
