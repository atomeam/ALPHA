...
export type ComponentType = 'card' | 'stat' | 'chart' | 'list' | 'info' | 'alert' | 'status' | 'action';
...
export const PrimitiveRenderer = ({ spec, theme, onAction }: { spec: ComponentSpec, theme?: any, onAction?: (command: string) => void }) => {
  const { type, title, props = {} } = spec;
  const borderClass = theme?.border === 'glass' ? 'border-glass rounded-3xl backdrop-blur-xl bg-white/[0.02]' : theme?.border === 'sharp' ? 'border-sharp' : 'border-rounded rounded-xl';
  const accentColor = theme?.primary || '#c4a661';

  const Title = ({ children, icon }: { children: React.ReactNode, icon?: string }) => (
    <div className="flex items-center justify-between mb-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className={cn(
          ...
          theme?.font === 'Mono' ? 'font-mono' : theme?.font === 'Serif' ? 'font-serif' : 'font-sans'
        )}>
          {children}
        </h3>
      </div>
  ...
  switch (type) {
      ...
      return (
        ...
        >
          ...
          <div className="flex flex-col gap-1 relative z-10">
            ...
            <span className="text-[10px] text-white/20 uppercase tracking-[0.3em] font-mono">{props.label}</span>
          </div>
          {props.description && (
            ...
            <ResponsiveContainer width="100%" height="100%">
              {props.color === 'line' ? (
                <LineChart data={props.data}>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#050505', border: '1px solid rgba(255,255,255,0.1)', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em' }}
                ...
                <AreaChart data={props.data}>
                  ...
                  />
                  <Area type="monotone" dataKey="value" stroke={accentColor} fillOpacity={1} fill={`url(#grad-${spec.id})`} strokeWidth={1} />
                </AreaChart>
              ) : (
          ...
          <div className="space-y-4">
            {props.items?.map((item, i) => (
              <div key={i} className="flex items-center gap-4 group/item">
                <div className="text-[9px] font-mono opacity-20 group-hover/item:opacity-60 transition-opacity">{(i + 1).toString().padStart(2, '0')}</div>
                <div className="flex-1 border-b border-white/5 pb-2 group-hover/item:border-white/10 transition-colors">
                  <span className="text-[11px] text-white/50 group-hover/item:text-white/90 transition-colors font-light tracking-wide uppercase">{item}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        >
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
              <span className="text-3xl font-mono text-white/90 leading-none tracking-tighter">{props.value}</span>
              <span className="text-[9px] font-bold text-white/20 uppercase tracking-[0.2em] mt-1">{props.label}</span>
            </div>
          </div>
          {/* Logic Pulse */}
    ...
    case 'action':
      return (
        ...
        >
          <Title icon={props.icon}>{title}</Title>
          <p className="text-[11px] text-white/40 leading-relaxed font-light mb-2">{props.description}</p>
          <button 
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
               ...
               <span className="text-[10px] font-bold text-red-400 uppercase tracking-widest">{title}</span>
            </div>
            <span className="text-[8px] font-mono text-red-500/40">CRITICAL_V_SYS</span>
          </div>
          <p className="text-[11px] text-red-200/40 leading-relaxed font-light italic">{props.description}</p>
      ...
      return (
        ...
        >
          ...
          <p className="text-[11px] text-white/40 leading-relaxed font-light tracking-wide italic border-l border-white/5 pl-4">
            {props.description || 'System initialized at root level. Recursive architectural memory engaged.'}
          </p>
        </motion.div>
      );
  }
